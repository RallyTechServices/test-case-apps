Ext.define('Rally.technicalservices.ProgressBarTemplate', {
     ...
     */
    calculateColorFn: function(values, key) {
    ...
    generateLabelTextFn: function (recordData) {
        return Ext.String.format('{0} of {1} Test Cases Run (<b>{2}%</b>)', recordData.count, recordData.total, this.calculatePercent(recordData));
    },
    ...
    config: {
        ...
        calculateWidth: function (recordData) {
            console.log('calculatewidth', recordData);
        ...
        calculatePercent: function (recordData) {
            ...
            var total = recordData.total || 0;

            if (total){
                var percentDone = (recordData.count || 0) / total;
    ...
    constructor: function(config) {
      ...
      var templateConfig = config && config.template || [
              ...
              '<div class="progress-bar-container field-{[this.percentDoneName]} {[this.getClickableClass()]} {[this.getContainerClass(values)]}" style="{[this.getDimensionStyle()]}">',
                '<tpl for=".">',
                 '<div class="rly-progress-bar" style="background-color: {[values.color]}; width: {[this.calculateWidth(values)]}; "></div>',
        ...
        // var templateConfig = config && config.template || [
        //         '<tpl if="this.shouldShowPercentDone(values)">',
        //         '<div class="progress-bar-container field-{[this.percentDoneName]}" style="{[this.getDimensionStyle()]}">',
